...
void menuEstadisticas(int partidas_totales, Partidas partidas[], usuarios registro[], int total_users){
	...
	int opcion, opcion2, masJugado[5], masJugadoTemp[5]={0}, masJugado_mayor, menosJugado, masGanadores[total_users][5], id=0;
	for(int i=0; i<partidas_totales; i++){
...
void levantarDatos(usuarios registro[], Partidas partidas[], int &id, int &partidas_totales) {
	ifstream levantar;	
	levantar.open("usuarios.txt");//guardamos en texto solamente los datos de usuarios
	
	if(levantar.good()) {
		levantar >> registro[id].nombre >> registro[id].contra;
		levantar >> registro[id].ultimo_acceso.dia >> registro[id].ultimo_acceso.mes >> registro[id].ultimo_acceso.anio >> registro[id].ultimo_acceso.hora >> registro[id].ultimo_acceso.min;
		while(!levantar.eof()){
			id++;
			levantar >> registro[id].nombre >> registro[id].contra;
			levantar >> registro[id].ultimo_acceso.dia >> registro[id].ultimo_acceso.mes >> registro[id].ultimo_acceso.anio >> registro[id].ultimo_acceso.hora >> registro[id].ultimo_acceso.min;
	...
	levantar.open("partidas.txt");//vamos desde el struct usuarios, que tiene adentro todas las partidas
	if(levantar.good()) {
		levantar >> partidas[partidas_totales].nombre_usuario >> partidas[partidas_totales].id_juego >> partidas[partidas_totales].puntaje_obtenido;
		while(!levantar.eof()){
			partidas_totales++;
			levantar >> partidas[partidas_totales].nombre_usuario >> partidas[partidas_totales].id_juego >> partidas[partidas_totales].puntaje_obtenido;
		}
	...
	levantar.open("premios.txt");
	if(levantar.good()) {
		int aux=0;
		
		for(int i=0; i<5; i++)
			levantar >> registro[aux].total[i].diplomas >> registro[aux].total[i].monedas >> registro[aux].total[i].medallas >> registro[aux].total[i].trofeos >> registro[aux].total[i].copas; 
...
void guardarDatos(usuarios registro[], Partidas partidas[], const int id, const int partidas_totales) {
	...
	for (int i = 0 ; i < id ; i++){
		guardar << registro[i].nombre << " " << registro[i].contra <<  " " << 
			registro[i].ultimo_acceso.dia << " " << registro[i].ultimo_acceso.mes << " " << registro[i].ultimo_acceso.anio << " " << registro[i].ultimo_acceso.hora << " " << registro[i].ultimo_acceso.min << endl;
	}
	guardar.close();
	
	guardar.open("partidas.txt");
	for (int i = 0 ; i < partidas_totales ; i++){
		guardar << partidas[i].nombre_usuario <<  " " << partidas[i].id_juego <<  " " << partidas[i].puntaje_obtenido << endl;
	}
	guardar.close();
	
	guardar.open("premios.txt");
	for(int i=0; i<id; i++){
		for(int j=0; j<5; j++)
			guardar << registro[i].total[j].diplomas << " " << registro[i].total[j].monedas << " " << registro[i].total[j].medallas << " " << registro[i].total[j].trofeos << " " << registro[i].total[j].copas << endl;	 
	}
	guardar.close();
}
